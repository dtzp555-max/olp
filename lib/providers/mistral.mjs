...
async function* _spawnAndStream(irRequest, authContext, spawnImpl) {
  ...
  // writing — vibe --prompt reads from CLI args, but stdin.end() can still
  // surface EPIPE if the process exits before the fd is flushed).
  // Port of OCP server.mjs:780.
  // Authority: Mistral Vibe CLI https://docs.mistral.ai/mistral-vibe/terminal/quickstart
  // observed behaviour — fast-failing spawn closes stdin; Node Writable surfaces EPIPE.
