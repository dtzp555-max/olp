...
async function _withKeyLock(id, fn) {
  ...
  // Chain: anyone joining sees prev → next as their wait point.
  _writeLocks.set(id, prev.then(() => next));
  ...
  try {
    ...
    release();
    // If no one queued after us, drop the entry to keep the Map bounded.
